...
class Node(object):
    ...
    def remove_dead_contacts(self):
        ...
        for c in move_remove_contacts:
            ...
            print('remove_dead_contacts:', self, c)
    ...
    # message
    #
    def build_message(self, protocol_major_version, protocol_minor_version, protocol_message_type,
                      protocol_command_code, obj):
    ...
    def build_packs(self, message_data):
        ...
        for s in range(0, len(message_data), step):
            ...
            pack = self.build_pack(message_id, len(message_data), message_n_packs, len(pack_data), pack_index,
                                   pack_data)
            pack_index += 1
            yield pack
    ...
    def parse_message(self, message, remote_host, remote_port):
        ...
        protocol_version_major, protocol_version_minor, protocol_message_type, protocol_command_code = struct.unpack(
            '!BBBB', message_header)
        protocol_command = self.get_protocol_command(protocol_version_major, protocol_version_minor,
                                                     protocol_command_code)
